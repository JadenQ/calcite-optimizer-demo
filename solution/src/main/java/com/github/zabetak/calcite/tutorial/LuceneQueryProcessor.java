...
import static org.apache.calcite.util.trace.CalciteTrace.getPlannerTaskTracer;
import static org.apache.calcite.util.trace.CalciteTrace.getPlannerTracer;
...
public class LuceneQueryProcessor {
  ...
  private static final Logger logger = org.slf4j.LoggerFactory.getLogger(LuceneQueryProcessor.class);
  ...
  public enum Type {
    ...
    PUSHDOWN,
    /**
     * Multi Join query scripts to be detected and report.
     */
    MULTI_JOIN
  }
  ...
  public static <T> Enumerable<T> execute(String sqlQuery, Type processorType)
          throws SqlParseException, FileNotFoundException {
...
//    RelOptPlanner planner = cluster.getPlanner();
    ...
    case PUSHDOWN:
      ...
      // lets compare simple and pushdown
//      planner.addRule(EnumerableRules.ENUMERABLE_TABLE_SCAN_RULE);
    ...
    System.out.println("Planner使用的规则："+planner.getRules());
    System.out.println("获得Digest: "+logPlan.getDigest());
    PrintWriter pw =null;
//    planner.dump(pw);
//    System.out.println("dump:"+planner.dump(pw));3

    System.out.println("to dot, before opt:"+ planner.toDot());

    try (PrintWriter out = new PrintWriter("D:\\docs\\task_analysis\\cec_project\\calcite-tutorial-boss21\\memo\\memo_before.dot")) {
      planner.dump(out);
      // dot -Tpng ./memo/memo_PUSHDOWN.dot -o ./memo/memo_PUSHDOWN.png, start with graph G
    }
...
//    System.out.println();
    ...
    System.out.println("to dot, after opt:"+ planner.toDot());
    try (PrintWriter out = new PrintWriter("D:\\docs\\task_analysis\\cec_project\\calcite-tutorial-boss21\\memo\\memo_"+processorType+".dot")) {
      planner.dump(out);
      // dot -Tpng ./memo/memo_PUSHDOWN.dot -o ./memo/memo_PUSHDOWN.png, start with graph G
    }
    ...
    System.out.println("Planner: "+getPlannerTaskTracer().getName());
    System.out.println(
            RelOptUtil.dumpPlan("[logical plan now: ]", logPlan, SqlExplainFormat.TEXT,
                    SqlExplainLevel.NON_COST_ATTRIBUTES));
